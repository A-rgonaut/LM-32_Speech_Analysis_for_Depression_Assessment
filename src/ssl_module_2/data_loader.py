...
class Dataset(TorchDataset):
        ...
        for audio_path in self.audio_paths:
            ...
            if self.config.segmentation_strategy == 'fixed_length':
                audio_segments_indices = segment_audio_sliding_window(
                    ...
                    max_utt_seconds=self.config.max_utt_seconds,
                    overlap_seconds=self.config.overlap_seconds,
                    min_utt_seconds=self.config.min_utt_seconds,
                    return_indices=True
                )
    ...
    def __getitem__(self, idx):
        ...
        target_length = int(self.config.max_utt_seconds * self.config.sample_rate)
        ...
        return {
            'input_values': segment.input_values.squeeze(0),
